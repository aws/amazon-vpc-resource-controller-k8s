...
func TestEni_InitResources(t *testing.T) {
	...
	assert.True(t, reflect.DeepEqual(map[string]*eni{ip1: expectedENIDetails1, ip2: expectedENIDetails1,
		ip3: expectedENIDetails2, prefix1: expectedENIDetails1}, manager.resourceToENIMap))
}
...
// TestEniManager_CreateIPV4Resource_TypeIPV4Address_FromSingleENI tests IPs are created using a single ENI when it has the desired capacity
