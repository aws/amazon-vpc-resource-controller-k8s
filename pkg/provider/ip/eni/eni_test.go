...
var (
	...
	ResourceTypeIPv4Address string = "IPv4Address"
	ResourceTypeIPv4Prefix  string = "IPv4Prefix"
)
...
func TestEni_InitResources(t *testing.T) {
	...
	assert.True(t, reflect.DeepEqual(map[string]*eni{ip1: expectedENIDetails1, ip2: expectedENIDetails1,
		ip3: expectedENIDetails2}, manager.resourceToENIMap))
}
...
// capacity
func TestEniManager_CreateIPV4Resource_FromSingleENI(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	mockEc2APIHelper.EXPECT().AssignIPv4ResourcesAndWaitTillReady(eniID1, ResourceTypeIPv4Address, 2).Return([]string{ip1, ip2}, nil)
	...
	ips, err := manager.CreateIPV4Resource(2, config.ResourceType(ResourceTypeIPv4Address), mockEc2APIHelper, log)
...
// TestEniManager_CreateIPV4Resource_FromMultipleENI tests IPs are assigned from multiple different ENIs
func TestEniManager_CreateIPV4Resource_FromMultipleENI(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	gomock.InOrder(
		mockEc2APIHelper.EXPECT().AssignIPv4ResourcesAndWaitTillReady(eniID1, ResourceTypeIPv4Address, 1).Return([]string{ip1}, nil),
		mockEc2APIHelper.EXPECT().AssignIPv4ResourcesAndWaitTillReady(eniID2, ResourceTypeIPv4Address, 1).Return([]string{ip2}, nil),
	)
...
// TestEniManager_CreateIPV4Resource_FromNewENI tests if existing ENIs cannot supply new IP, IPs are allocated from a new
// ENI
func TestEniManager_CreateIPV4Resource_FromNewENI(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	ips, err := manager.CreateIPV4Resource(4, config.ResourceType(ResourceTypeIPv4Address), mockEc2APIHelper, log)
...
func TestEniManager_DeleteIPV4Address(t *testing.T) {
	...
	mockEc2APIHelper.EXPECT().UnassignIPv4Resources(eniID1, ResourceTypeIPv4Address, []string{ip1}).Return(nil)
	mockEc2APIHelper.EXPECT().UnassignIPv4Resources(eniID2, ResourceTypeIPv4Address, []string{ip3, ip4}).Return(nil)
	...
	failedToDelete, err := manager.DeleteIPV4Resource([]string{ip3, ip1, ip4}, config.ResourceType(ResourceTypeIPv4Address), mockEc2APIHelper, log)
...
func TestEniManager_DeleteIPV4Address_SomeFail(t *testing.T) {
	...
	mockInstance.EXPECT().PrimaryNetworkInterfaceID().Return(eniID1)
	// Unassign the IPs from interface 1 and 2
	mockEc2APIHelper.EXPECT().UnassignIPv4Resources(eniID1, ResourceTypeIPv4Address, []string{ip1}).Return(mockError)
	mockEc2APIHelper.EXPECT().UnassignIPv4Resources(eniID2, ResourceTypeIPv4Address, []string{ip3}).Return(nil)
	// Delete the network interface 2 as it has no more secondary IP left
	mockEc2APIHelper.EXPECT().DeleteNetworkInterface(&eniID2).Return(nil)

	failedToDelete, err := manager.DeleteIPV4Resource([]string{ip1, ip3}, config.ResourceType(ResourceTypeIPv4Address), mockEc2APIHelper, log)
...
func TestEniManager_groupIPsPerENI(t *testing.T) {
	...
	eniManager := eniManager{resourceToENIMap: ipToENIMap, attachedENIs: attachedENIs}

	groupedIP := eniManager.groupResourcesPerENI([]string{ip1, ip4, ip3})
