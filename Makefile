...
VERSION ?= $(GIT_VERSION)
IMAGE ?= $(REPO):$(VERSION)
BASE_IMAGE ?= public.ecr.aws/eks-distro-build-tooling/eks-distro-minimal-base-nonroot:latest.al2023
GOLANG_VERSION ?= $(shell cat .go-version)
BUILD_IMAGE ?= public.ecr.aws/docker/library/golang:$(GOLANG_VERSION)
GORUNNER_VERSION ?= public.ecr.aws/eks-distro-build-tooling/go-runner:v0.18.0-go-1.25-latest.al2023
GOARCH ?= amd64
