...
##  eksctl create cluster -f $DIR/test/integration-test/config/cluster.yaml || exit 1
...
#          --managed || exit 1
#
#  SGS=$(set-security-groups $CLUSTER_NAME)
#fi
#SGS=$(get-security-groups $CLUSTER_NAME)
#SG_ONE=$(echo $SGS | cut -d " " -f 1)
#SG_TWO=$(echo $SGS | cut -d " " -f 2)
#
#echo "***** will use security group $SG_ONE and/or $SG_TWO if they are not empty  *****"
#
## TODO: inject Trunck ENI policy into the ng iam role
...
#
## TODO: pick one of the following ways to do trunk ENI policy injection.
## TODO: add a trunck ENI iam policy into test account
...
#test-eni-pod-to-eni-pod "$NODE_ONE" "$NODE_TWO"
#sleep 10
...
# Not working now
#test-revoke-security-group-rules "$NODE_ONE" "$NODE_TWO"

#echo "*******************************************************************************"
#echo "Running integration tests on current image:"
#echo ""
#pushd $DIR/test/integration-test/test/integration
#GO111MODULE=on go test -v -timeout 0 ./... --kubeconfig=$KUBECONFIG --ginkgo.focus="\[cni20-integration\]" --ginkgo.skip="\[Disruptive\]" \
#    --assets=./assets
#TEST_PASS=$?
#popd
#
