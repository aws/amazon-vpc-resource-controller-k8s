...
TEST_DIR=/tmp/cni20-test/$(date "+%Y%M%d%H%M%S")-$TEST_ID
...
CLUSTER_NAME=cni20-test-$CLUSTER_ID
TEST_CLUSTER_DIR=/tmp/cni20-test/cluster-$CLUSTER_NAME
...
CNI_CONFIG_PATH="$DIR/../config/aws-k8s-cni.yaml"
#TEST_CONFIG_PATH="$TEST_CONFIG_DIR/aws-k8s-cni.yaml"

if [[ ! -f "$CNI_CONFIG_PATH" ]]; then
    echo "$CNI_CONFIG_PATH DOES NOT exist. Set \$CNI_TEMPLATE_VERSION to an existing directory in ./config/"
    exit
fi
...
#    echo "test image version $TEST_IMAGE_VERSION and local git version $LOCAL_GIT_VERSION"
...
#echo "Using $CNI_CONFIG_PATH as a template"
#
#cp "$CNI_CONFIG_PATH" "$TEST_CONFIG_PATH"
...
TEST_CONFIG_PATH=../config/aws-k8s-cni.yaml
...
$KUBECTL_PATH set daemonset aws-node ENABLE_POD_ENI=true -n kube-system
...
CONTROLLER_IMG=744053100597.dkr.ecr.us-west-2.amazonaws.com/next-cni:latest
IMG=$CONTROLLER_IMG
echo $CONTROLLER_IMG
echo $IMG
#
...
echo $DIR
#
make docker-build IMG=$IMG|| exit 1
echo "*****finished build and sleep 60s.*****"
sleep 60
echo "*****starting pushing image*****"
make docker-push IMG=$IMG || exit 1
...
kubectl apply -f $DIR/config/samples/vpcresources_v1beta1_securitygrouppolicy.yaml